...
| RN | Regra | Onde mora | Origem |
|---|---|---|---|
| RN-06 | O `numero` do corte deriva de `inicio_seg`, exceto quando a posição foi fixada à mão. | `domain/ordem_cortes.py` (`ordenar_por_tempo`, `pins_para_ordem`) | D-448 |
| RN-07 | **Duração líquida** = intervalo − desvios (corte e bloco) ou soma dos segmentos (short). `fim − inicio` mente quando há buracos. | `domain/juncao_cortes.py` (`duracao_liquida`); `domain/arranjo_blocos.py` (`duracao_liquida`); `domain/segmentos_short.py` (`duracao_liquida`); `services/pipeline_corte_fields.py` (`_duracao_layout_corte`) | D-575 |

...
|---|---|---|---|---|
| **Canal** | Identidade, tema, mascote, credenciais e skills editoriais de cada canal. | Canal (identidade no `settings.db`), skill editorial, tema | `domain/canal/identidade` e `domain/canal/mascote` (D-762), `channel_config_loader`, `editorial_skills`, `editorial_scaffolds`, `editorial_identity`, `services/channels`, `channel_paths` | Upstream de todos. Desde a D-666 não vaza mais para o `models.py`. |
| **Descoberta de lives** | Busca, ranking (VPH + recência) e fila de lives candidatas. | Live candidata | `domain/live_candidata/ranking_lives`, `services/ranking_lives`, `ranking_settings`, `routers/ranking_lives`, `routers/youtube_browser` | Promove uma candidata a Projeto. |
...
| **Render** | Grade, composição, overlays, fila do worker. | Job de render (fila em arquivo) | `services/render/remotion_render`, `services/render/pipeline_render`, `infrastructure/worker_queue`, `infrastructure/render/bruto_pipeline`, `infrastructure/render/overlay_codec` | Subdomínio técnico: serve Layout e Shorts. |
...
| **Shorts** | Candidatos, segmentos, enquadramento, palco vertical, gancho, legenda, render. | **Short** | `services/shorts`, `services/palco_shorts`, `services/render/render_short`, `domain/short/segmentos_short`, `domain/short/gancho_short`, `domain/short/palco_short`, `domain/short/metadados_short` | Downstream de Corte (tempo do bruto, RN-08) e de Layout (herança de palco, RN-11). |

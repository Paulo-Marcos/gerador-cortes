...
def test_campos_opcionais_aceitam_none(tmp_path: Path):
    ...
    llm_calls_store.gravar_llm_call(
        llm_calls_store.LlmCallRecord(etapa="sentimento-ranking", model="haiku", sucesso=True),
        db_path=db,
    )
...
def test_filtros_por_projeto_corte_e_etapa(tmp_path: Path):
    db = tmp_path / "llm_calls.db"
    llm_calls_store.gravar_llm_call(
        llm_calls_store.LlmCallRecord(etapa="cortador-expert", projeto_id="A"), db_path=db
    )
    llm_calls_store.gravar_llm_call(
        llm_calls_store.LlmCallRecord(etapa="metadados-expert", projeto_id="A", corte_id="c1"),
        db_path=db,
    )
    llm_calls_store.gravar_llm_call(
        llm_calls_store.LlmCallRecord(etapa="cortador-expert", projeto_id="B"), db_path=db
    )
...
def test_ordenado_por_ts_desc_e_respeita_limite(tmp_path: Path):
    ...
    for i in range(5):
        llm_calls_store.gravar_llm_call(
            llm_calls_store.LlmCallRecord(etapa=f"etapa-{i}", prompt=f"p{i}"), db_path=db
        )
...
def test_tabela_criada_de_forma_idempotente(tmp_path: Path):
    ...
    llm_calls_store.gravar_llm_call(llm_calls_store.LlmCallRecord(etapa="x"), db_path=db)
    llm_calls_store.inicializar(db)
...
async def test_rota_da_ultima_geracao_le_a_telemetria(telemetria_no_tmp: Path):
    llm_calls_store.gravar_llm_call(
        llm_calls_store.LlmCallRecord(etapa="cortes", corte_id="c1", model="gemini-2.5-pro")
    )
...
async def test_rota_da_lista_de_chamadas_filtra_pela_telemetria(telemetria_no_tmp: Path):
    llm_calls_store.gravar_llm_call(
        llm_calls_store.LlmCallRecord(etapa="cortes", projeto_id="A", model="claude-opus-5")
    )
    llm_calls_store.gravar_llm_call(
        llm_calls_store.LlmCallRecord(etapa="metadados", projeto_id="B", model="claude-opus-5")
    )

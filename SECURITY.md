...
- **Use numa máquina em que você confia.**
...
- **Só esta máquina alcança a API** (D-745): o backend escuta em `127.0.0.1`, o
  CORS só libera origens `localhost`, e uma guarda recusa, vindos de qualquer outra
  origem, métodos que mudam dados, WebSocket e `Host` estranho (DNS rebinding).
  Não exponha a porta do backend na rede (proxy, túnel, redirecionamento): sem
  login, isso devolve a API a quem a alcançar.

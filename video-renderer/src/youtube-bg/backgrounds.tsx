...
const ConcentricCircles: FC<{ size?: number; opacity?: number; dotted?: boolean }> = ({
  ...
  <svg width={size} height={size} viewBox="0 0 100 100" style={{ opacity }}>
    <circle cx="50" cy="50" r="48" fill="none" stroke={PALETTE.hud} strokeWidth="0.4" strokeDasharray={dotted ? "0.5 1.5" : "none"} />
    <circle cx="50" cy="50" r="36" fill="none" stroke={PALETTE.hud} strokeWidth="0.4" />
    <circle cx="50" cy="50" r="24" fill="none" stroke={PALETTE.hud} strokeWidth="0.3" strokeDasharray="0.5 2" />
    <circle cx="50" cy="50" r="12" fill="none" stroke={PALETTE.hud} strokeWidth="0.3" />
    <circle cx="50" cy="50" r="1" fill={PALETTE.hud} />
  </svg>
);

const DotMatrix: FC<{ rows?: number; cols?: number; gap?: number; dot?: number; opacity?: number; color?: string }> = ({
  rows = 6,
  cols = 10,
  gap = 18,
  dot = 2,
  ...
  <svg width={cols * gap} height={rows * gap} style={{ opacity }}>
    {Array.from({ length: rows }).flatMap((_, r) =>
      Array.from({ length: cols }).map((_, c) => (
        <circle key={`${r}-${c}`} cx={c * gap + dot} cy={r * gap + dot} r={dot / 2} fill={color} />
      )),
    )}
  </svg>
);
...
const TopoLines: FC<{ opacity?: number; strokeWidth?: number; rings?: number; spin?: number; secondary?: boolean }> = ({
  opacity = 0.5,
  strokeWidth = 1.1,
  rings = 14,
  spin = 3,
  ...
  const id = useId();
  return (
    <svg width="100%" height="100%" viewBox="0 0 1920 1080" preserveAspectRatio="none" style={{ position: "absolute", inset: 0, opacity }}>
      <defs>
      ...
      <g stroke={PALETTE.hud} fill="none" strokeWidth={strokeWidth} mask={`url(#${id}-mask)`}>
        {Array.from({ length: rings }).map((_, i) => {
          const r = 120 + i * 80;
          return (
            <ellipse key={i} cx="960" cy="540" rx={r * 1.1} ry={r * 0.7} transform={`rotate(${i * spin} 960 540)`} strokeDasharray={i % 3 === 0 ? "none" : "5 7"} />
          );
        })}
        {secondary &&
          Array.from({ length: 8 }).map((_, i) => {
            const r = 60 + i * 50;
            return <ellipse key={`b${i}`} cx="280" cy="820" rx={r * 1.2} ry={r * 0.8} opacity="0.6" />;
          })}
      </g>
    </svg>
  );
...
const BgHudForte: FC = () => (
  <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse 80% 70% at 50% 50%, rgba(34,52,42,0.75) 0%, rgba(10,16,12,1) 75%), linear-gradient(180deg, #0e1410 0%, #060a08 100%)` }}>
    <HudGrid size={160} opacity={0.32} />
    <HudGrid size={32} opacity={0.1} />
    <div style={{ position: "absolute", top: 100, right: 320 }}><ConcentricCircles size={220} opacity={0.5} /></div>
    <div style={{ position: "absolute", top: "45%", right: -220, transform: "translateY(-50%)" }}><Semicircles size={520} opacity={0.55} /></div>
    <div style={{ position: "absolute", top: "60%", left: -240, transform: "translateY(-50%)" }}><Semicircles size={460} opacity={0.35} side="left" /></div>
    <div style={{ position: "absolute", bottom: 70, left: 320 }}><DotMatrix rows={6} cols={10} gap={16} dot={2.4} opacity={0.55} /></div>
    <div style={{ position: "absolute", top: 200, left: 0, right: 0, display: "flex", justifyContent: "center" }}><TickRow count={28} /></div>
    <Noise opacity={0.1} />
  </div>
);

const BgTopoEstrutural: FC = () => (
  <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse 70% 60% at 50% 50%, rgba(32,46,38,0.55) 0%, transparent 65%), radial-gradient(ellipse 50% 50% at 78% 60%, rgba(58,38,28,0.35) 0%, transparent 60%), linear-gradient(180deg, #0a100d 0%, #060a08 100%)` }}>
    <TopoLines opacity={0.45} strokeWidth={1.1} rings={14} />
    <div style={{ position: "absolute", top: 200, left: 0, right: 0, display: "flex", justifyContent: "center" }}><TickRow count={20} /></div>
    <Noise opacity={0.1} />
    <Vignette />
  </div>
);
...
const BgHudTopoFusion: FC = () => (
  <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse 75% 65% at 50% 50%, rgba(32,48,40,0.70) 0%, rgba(10,14,11,1) 78%), linear-gradient(180deg, #0c120e 0%, #060a08 100%)` }}>
    <HudGrid size={160} opacity={0.18} />
    <TopoLines opacity={0.4} strokeWidth={1.0} rings={11} spin={4} />
    <div style={{ position: "absolute", top: 100, right: 300 }}><ConcentricCircles size={200} opacity={0.4} dotted /></div>
    <div style={{ position: "absolute", top: "45%", right: -200, transform: "translateY(-50%)" }}><Semicircles size={480} opacity={0.45} /></div>
    <div style={{ position: "absolute", bottom: 70, left: 320 }}><DotMatrix rows={5} cols={9} gap={16} dot={2.2} opacity={0.45} /></div>
    <Noise opacity={0.1} />
  </div>
);

const BgArchitecturalHud: FC = () => (
  <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse 90% 70% at 50% 50%, rgba(28,42,34,0.85) 0%, rgba(12,18,14,1) 75%), linear-gradient(180deg, #0e1410 0%, #080c09 100%)` }}>
    <HudGrid size={160} opacity={0.3} />
    <div style={{ position: "absolute", top: 90, right: 280 }}><ConcentricCircles size={200} opacity={0.45} /></div>
    <div style={{ position: "absolute", top: "50%", right: -180, transform: "translateY(-50%)" }}><Semicircles size={420} opacity={0.45} /></div>
    <div style={{ position: "absolute", bottom: 70, left: 280 }}><DotMatrix rows={5} cols={9} gap={16} dot={2.4} opacity={0.55} /></div>
    <div style={{ position: "absolute", top: 200, left: 0, right: 0, display: "flex", justifyContent: "center" }}><TickRow count={24} /></div>
    <Noise opacity={0.1} />
  </div>
);
...
const BgCosmograph: FC = () => {
  const id = useId();
  return (
    <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse 80% 70% at 50% 50%, rgba(32,46,38,0.55) 0%, transparent 65%), radial-gradient(ellipse 40% 50% at 80% 55%, rgba(58,38,28,0.35) 0%, transparent 60%), linear-gradient(180deg, #0a100d 0%, #060a08 100%)` }}>
      <svg width="100%" height="100%" viewBox="0 0 1920 1080" preserveAspectRatio="none" style={{ position: "absolute", inset: 0, opacity: 0.55 }}>
        <defs>
...
export const YoutubeBackground: FC<{ fundo?: YoutubeBackgroundId }> = ({ fundo }) => {
  const Bg = COMPONENTS[fundo ?? DEFAULT_YOUTUBE_BACKGROUND] ?? COMPONENTS[DEFAULT_YOUTUBE_BACKGROUND];
  return <Bg />;
};

...
function useTrechosRecolhidos(): [boolean, () => void] {
  ...
  const alternar = useCallback(() => {
    setRecolhidos((atual) => {
      try {
        window.localStorage.setItem(TRECHOS_RECOLHIDOS_KEY, atual ? '0' : '1');
      } catch {
...
export function EditorFase1({
  ...
  const [trechosRecolhidos, alternarTrechos] = useTrechosRecolhidos();
  ...
  return (
    <div className="flex h-full min-w-0">
      ...
      {trechosRecolhidos ? (
        <div className="flex flex-none flex-col items-center py-2 pl-2">
          <button
            type="button"
            className="btn btn-icon btn-ghost"
            onClick={alternarTrechos}

/**
 * Port fiel do Angular CenaPreviewComponent → React.
 * Mantém exatamente as mesmas cores, tamanhos e posições do Remotion.
 * IMPORTANTE: ao alterar uma cena no video-renderer, atualizar aqui também.
 */
...
function cluster(extra: string): CSSProperties {
  return { position: 'absolute', display: 'flex', alignItems: 'center', ...css(extra) };
}

function textoTitulo(size: number, weight: number): CSSProperties {
  return {
    fontSize: size,
    fontWeight: weight,
    color: C.brancoNeve,
...
function Mascote({ tamanho, mood }: { tamanho: string; mood?: string }) {
...
// ---------------------------------------------------------------------------
// Componente público — mede o container e escala o stage 1920×1080
// ---------------------------------------------------------------------------
